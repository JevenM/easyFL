...
def table_cifar(filters={'lr': '0.1'}, path='../task/cifar10/record/'):
    ...
    for head in heads:
        ...
        print("& {:.2f}$\\pm${:.2f}".format(
            np.mean(means), np.std(means)), end=' ')
        print("& {:.2f}$\\pm${:.2f}".format(
            np.mean(vars), np.std(vars)), end=' ')
        print("& {:.2f}$\\pm${:.2f}".format(
            np.mean(worstKs), np.std(worstKs)), end=' ')
        print("& {:.2f}$\\pm${:.2f}\\\\".format(
            np.mean(bestKs), np.std(bestKs)))
    return 0
...
def table_order(dataset):
    ...
    for head in heads:
        ...
        for f in fnames:
            ...
            vars_.append(cdata['var_curve'][-1])
        print("{:.2f}$\\pm${:.2f} &".format(
            np.mean(vars_), np.std(vars_)), end=' ')
...
def data_for_plot(filters={'lr': '0.1'}, path='.', methods=[]):
    ...
    for method in methods:
        ...
        for f in fnames:
            ...
            vars_.append(cdata['var_curve'])
        outdict[method] = {}
        # averaging results of each method with different random seeds
        outdict[method]['ave'] = np.mean(means, axis=0)
        outdict[method]['var'] = np.mean(vars_, axis=0)
    return outdict


def draw_data(dataset, methods=[], labels=[], curve='ave'):
    colors = ['m', 'g', 'y', 'c', 'b', 'r',
              'seagreen', 'darkblue', 'darkgreen', 'darkred']
    data = data_for_plot({'lr': '0.01'}, 'task/' +
                         dataset + '/record/', methods)
    if dataset == 'cifar10':
        R = 2000
    ...
    for i, key in enumerate(data.keys()):
        plt.plot(x, np.asarray(data[key][curve][:-2]),
                 colors[i], label=labels[i], linewidth='2')
...
if __name__ == '__main__':
    ...
    methods = [
        'fedavg',
        'qfedavg_q1',
        'fedfa_beta0.5_gamma0.5_momentum0.9',
        'fedmgda+_epsilon0.1',
        'fedfv_alpha0.1_tau0',
        'fedfv_alpha0.1_tau10',
    ]
